...
PROVIDES = "\
    ...
    virtual/libiconv \
    glibc-thread-db \
    ${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-atto', 'linux-libc-headers', '', d)} \
    ${@base_conditional('PREFERRED_PROVIDER_gdbserver', 'external-toolchain-atto', 'gdbserver', '', d)} \
"

DEPENDS = "${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-atto', '', 'linux-libc-headers', d)}"
...
do_install() {
    ...
    ${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-atto', '', 'rm -rf ${D}/usr/include/linux', d)}

    cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/include/* ${D}${includedir}
    ${@base_conditional('PREFERRED_PROVIDER_gdbserver', 'external-toolchain-atto', '', 'rm -rf ${D}/usr/bin/gdbserver', d)}
